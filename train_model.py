...
flags.DEFINE_integer('batch_size',50,'the batch_size of the training procedure')
flags.DEFINE_float('lr',0.1,'the learning rate')
flags.DEFINE_float('lr_decay',0.6,'the learning rate decay')
flags.DEFINE_integer('vocabulary_size',100000,'vocabulary_size')
...
flags.DEFINE_integer('max_len',300,'max_len of training sentence')
...
flags.DEFINE_integer('class_num',20,'class num')
...
def train_step():
    ...
    train_data,valid_data,test_data=data_helper.load_data(FLAGS.max_len,FLAGS.batch_size)
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        with tf.variable_scope("model",reuse=None,initializer=initializer):
            model = LSTM_Model(config=config,is_training=True)

        with tf.variable_scope("model",reuse=True,initializer=initializer):
            valid_model = LSTM_Model(config=config,is_training=False)
            test_model = LSTM_Model(config=eval_config,is_training=False)
